#!/usr/bin/env Rscript
# Filter chi2_context output: show only sections where ALL specified contexts
# rank within the top N standardized residuals for the given mutation type,
# where N = number of specified contexts.
#
# Usage: Rscript filter_top2_context.R <chi2_output_file> <mut_type> <ctx1> <ctx2> [ctx3 ...]
# Example:
#   Rscript filter_top2_context.R cnidaria_chi2_context.txt "C>A" "A[C>A]A" "A[C>A]G"

args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 4) {
  stop("Usage: Rscript filter_top2_context.R <chi2_output_file> <mut_type> <ctx1> <ctx2> [ctx3 ...]")
}
...
for (line in lines) {
  ...
  # Mutation type summary line, e.g.: "C>A  chisq  218.26  15  4.03e-38  **"
